...
# 3.	Оценить модель с помощью метода наименьших квадратов. 
...
# 4. Проверьте построенную в 3 п. модель на удовлетворение осттков условиям Гаусса-Маркова 
# (равенство нулю матожидания остатков, нормальность остатков, гетероскедастичность, автокорреляцию). Сделайте выводы.
...
library(nortest)
ks.test(residuals(lw6), "pnorm", mean = mean(residuals(lw6), na.rm = T), 
        sd = sd(residuals(lw6), na.rm = T))

# гомоскедастичность
library("lmtest")
bptest(lw6, varformula = NULL, studentize = FALSE, villa2)

# автокорреляцию 
durbinWatsonTest((lw6))